...
const comicInfoContainer = document.querySelector(
  ".comic-information__container"
);
  ...
  return `<article class="character-card card" >
         <div class="character-card__image-container">
            <img class="character-card__image" data-id="${
              character.id
            }" src="${checkIfThereIsImageInCharacter(
    character,
    "/portrait_uncanny"
  )}"/>
        </div>
  ...
  return `<article class="comic-card card">
        <div class="comic-card__image-container">
            <img class="comic-card__image" data-id="${
              comic.id
            }" src="${checkIfThereIsImageInComic(comic)}"/>
        </div>
...
const showInfoInHTML = (info, collection) => {
  ...
  info.data.results.map((comicOrCharacter) => {
    ...
    if (collection == "comics") {
      return (cardsContainer.innerHTML += showComicsCards(comicOrCharacter));
    }
  });
};
...
const showInformationFromApi = async (
  ...
  const response = await fetch(
    `${baseUrl}${collection}?apikey=${apiKey}&orderBy=${orderBy}${checkInputSearch(
      inputSearch,
      collection
...
const showCharacterCardInformation = (character) => {
  character.data.results.map((character) => {
    ...
    characterInfoContainer.innerHTML = `<div class="character-information__image">
...
const showComicCardInformation = (comic) => {
  comic.data.results.map((comic) => {
    ...
    comicInfoContainer.innerHTML = `<div class="comic-information__image">
                    ...
                    <div class="comic-information">
                        ...
                        <p>18/12/2019</p>
                        <h3>Guionistas:</h3>
                        <p class="writers">${checkIfThereIsWritersInformation(
                          comic
                        )}</p>
                        <h3>Descripción:</h3>
...
const showExtraInfo = async (url, collection) => {
  const response = await fetch(
    `${url}?apikey=${apiKey}&offset=${currentPage * resultsPerPage}`
  );
...
const checkPages = (collection) => {
  collection.data.total > 20 ? ableNextAndLast() : disabledPages();
};

const showComicsOfCharacter = (comics) => {
  checkPages(comics);
  cardsContainer.innerHTML = ``;
  comics.data.results.length > 0 ? showComics(comics) : showNoResults();
};
...
const showComics = (comics) => {
  comics.data.results.map((comic) => {
    ...
    cardsContainer.innerHTML += showComicsCards(comic);
    enableOrDisablePages(offset, totalQuantityOfCollection);
    calculateAllPages();
...
const showCharacters = (characters) => {
  characters.data.results.map((character) => {
    ...
    cardsContainer.innerHTML += showCharactersCards(character);
    enableOrDisablePages(offset, totalQuantityOfCollection);
    calculateAllPages();
...
const alphabethicNewestSearch = document.querySelector(
  "#alphabetic-newest-search"
);
...
const checkWhichUrlNeedsToFetch = () => {
  if (
    ...
    characterInfoContainer.classList.contains("hidden")
  ) {
    showInformationFromApi(
      collectionSearch.value,
      alphabethicNewestSearch.value,
      checkIfThereIsAnInputSearch()
    );
  } else {
    comicInfoContainer.classList.contains("hidden")
      ? showExtraInfo(urlWithComics, "characters")
...
const enableOrDisableFirstAndPrevious = () => {
  currentPage == 0 ? disableFirstAndPrevious() : ableFirstAndPrevious();
};
